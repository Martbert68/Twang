...
void init_n(struct node **n ,struct node **m)
{
	...
	mm=25;
	mass[0]=mm; //G
	mass[1]=mm/pow(pow(pp,7),2); //D
	mass[2]=mm/pow(pow(pp,14),2); //A
	mass[3]=mm/pow(pow(pp,21),2); //E
...
void do_n(struct node **n ,struct node **m)
{
	...
	for (i=1;i<len+1;i++)
	{
		...
		if (i==BRIDGE)
		{
		xa=((n[i-1]->x+n[i+1]->x)-(2*n[i]->x))/50;
		ya=((n[i-1]->y+n[i+1]->y)-(2*n[i]->y))/50;
		za=((n[i-1]->z+n[i+1]->z)-(2*n[i]->z))/50;
		aa=((n[i-1]->a+n[i+1]->a)-(2*n[i]->a))/50;
		} else{ 
	...
	for (i=1;i<len+1;i++)
	{
		if (i==BRIDGE)
		{
		xa=((m[i-1]->x+m[i+1]->x)-(2*m[i]->x))/50;
		ya=((m[i-1]->y+m[i+1]->y)-(2*m[i]->y))/50;
		za=((m[i-1]->z+m[i+1]->z)-(2*m[i]->z))/50;
		aa=((m[i-1]->a+m[i+1]->a)-(2*m[i]->a))/50;
		}else{ 
	...
	n[BRIDGE]->xv+=(-xx+yy+zz+aa+bb+cc)/6900;
	n[BRIDGE]->yv+=(xx-yy+zz+aa+bb+cc)/6900;
	n[BRIDGE]->zv+=(xx+yy-zz+aa+bb+cc)/6900;
	n[BRIDGE]->av+=(xx+yy+zz-aa+bb+cc)/6900; 
...
void draw_n (unsigned char *image, struct node **n,int frame)
{
	...
	r=128+(127*(sin((double)(i-frame)/40))); g=128+(127*(cos((double)(i+(frame*3))/37))); b=128+(127*(sin((double)(i+frame)/5)));
	qlott(image, i*X_SIZE/len, (n[i]->x/4)+(Y_SIZE/7), r,g,b, 1,12);
	r=128+(127*(sin((double)(i-frame)/41))); g=128+(127*(cos((double)(i+(frame*3))/39))); b=128+(127*(sin((double)(i+frame)/7)));
	qlott(image, i*X_SIZE/len, (n[i]->y/4)+(2*Y_SIZE/7), r,g,b, 1,10);
	r=128+(127*(sin((double)(i-frame)/49))); g=128+(127*(cos((double)(i+(frame*3))/31))); b=128+(127*(sin((double)(i+frame)/4)));
	qlott(image, i*X_SIZE/len, (n[i]->z/4)+(3*Y_SIZE/7), r,g,b, 1,8);
	r=128+(127*(sin((double)(i-frame)/42))); g=128+(127*(cos((double)(i+(frame*3))/32))); b=128+(127*(sin((double)(i+frame)/6)));
...
int main(int argc,char *argv[])
{
  ...
  if ((list= fopen("tune.lst", "r")) == NULL) {
	...
	frames=30;
        ...
        wav=(short *)malloc(sizeof (short)*120*rate*dur); // font
	...
	if (total%(rate)==0){ss=1;} 
	if (total%(rate/2)==0 || (pluck[pc][6]==1 && total%(rate/4)==0))
	{
	
	       	//for (j=0;j<6;j++){fret[j]=len;}	
		px=0;py=0;pz=0;pa=0;
	 	if (pluck[pc][3]>-1){ px=1; fret[0]=len/pow(pp,pluck[pc][3]);}
	 	if (pluck[pc][2]>-1){ py=1; fret[1]=len/pow(pp,pluck[pc][2]);}
	 	if (pluck[pc][1]>-1){ pz=1; fret[2]=len/pow(pp,pluck[pc][1]);}
	 	if (pluck[pc][0]>-1){ pa=1; fret[3]=len/pow(pp,pluck[pc][0]);}
		printf ("Note %d \n",pc);
		pc++;
		ss=0;
		if (pc>nc){go=0;}
	...
	if (px && n[DOWN]->xv>=0  && n[DOWN]->xv < 10){ n[DOWN]->xv+=((double)(rand()%19001/10000));}
	if (py && n[DOWN]->yv>=0  && n[DOWN]->yv < 10){ n[DOWN]->yv+=((double)(rand()%19001/10000));}
	if (pz && n[DOWN]->zv>=0  && n[DOWN]->zv < 10){ n[DOWN]->zv+=((double)(rand()%19001/10000));}
	if (pa && n[DOWN]->av>=0  && n[DOWN]->av < 10){ n[DOWN]->av+=((double)(rand()%19001/10000));}
	...
	double   ja,jp;
	s=0;t=0;
	ja=1;jp=1;s+=n[BRIDGE]->x*ja; t+=n[BRIDGE]->x*jp;
	ja=1;jp=1;s+=n[BRIDGE]->y*ja; t+=n[BRIDGE]->y*jp;
	ja=1;jp=1;s+=n[BRIDGE]->z*ja; t+=n[BRIDGE]->z*jp;
	ja=1;jp=1;s+=n[BRIDGE]->a*ja; t+=n[BRIDGE]->a*jp;

...
void do_n(struct node **n ,struct node **m)
{
	...
	for (i=1;i<len+1;i++)
	{
		...
		if (i==BRIDGE)
		{
		xa=((n[i-1]->x+n[i+1]->x)-(2*n[i]->x))/500;
		ya=((n[i-1]->y+n[i+1]->y)-(2*n[i]->y))/500;
		za=((n[i-1]->z+n[i+1]->z)-(2*n[i]->z))/500;
		aa=((n[i-1]->a+n[i+1]->a)-(2*n[i]->a))/500;
		} else{ 
	...
	for (i=1;i<len+1;i++)
	{
		if (i==BRIDGE)
		{
		xa=((m[i-1]->x+m[i+1]->x)-(2*m[i]->x))/500;
		ya=((m[i-1]->y+m[i+1]->y)-(2*m[i]->y))/500;
		za=((m[i-1]->z+m[i+1]->z)-(2*m[i]->z))/500;
		aa=((m[i-1]->a+m[i+1]->a)-(2*m[i]->a))/500;
		}else{ 
...
void draw_n (unsigned char *image, struct node **n,int frame)
{
	...
	r=128+(127*(sin((double)(i-frame)/40))); g=128+(127*(cos((double)(i+(frame/3))/37))); b=128+(127*(sin((double)(i+frame)/59)));
	qlott(image, i*X_SIZE/len, (n[i]->x/4)+(Y_SIZE/7), r,g,b, 1,12);
	r=128+(127*(sin((double)(i-frame)/41))); g=128+(127*(cos((double)(i+(frame/3))/39))); b=128+(127*(sin((double)(i+frame)/51)));
	qlott(image, i*X_SIZE/len, (n[i]->y/4)+(2*Y_SIZE/7), r,g,b, 1,10);
	r=128+(127*(sin((double)(i-frame)/49))); g=128+(127*(cos((double)(i+(frame/3))/31))); b=128+(127*(sin((double)(i+frame)/48)));
	qlott(image, i*X_SIZE/len, (n[i]->z/4)+(3*Y_SIZE/7), r,g,b, 1,8);
	r=128+(127*(sin((double)(i-frame)/42))); g=128+(127*(cos((double)(i+(frame/3))/32))); b=128+(127*(sin((double)(i+frame)/41)));
...
int main(int argc,char *argv[])
{
  ...
  if ((list= fopen("tune.lst", "r")) == NULL) {
	...
	if (total%(rate)==0){ss=400;} 
	if (total%(rate/4)==0 || (pluck[pc][6]==1 && total%(rate/8)==0))
	{
	
	       	//for (j=0;j<6;j++){fret[j]=len;}	
	 	if (pluck[pc][3]>-1){ n[DOWN]->xv=(400+ss)/sqrt(mass[0]); fret[0]=len/pow(pp,pluck[pc][3]);}
	 	if (pluck[pc][2]>-1){ n[DOWN]->yv=(400+ss)/sqrt(mass[1]); fret[1]=len/pow(pp,pluck[pc][2]);}
	 	if (pluck[pc][1]>-1){ n[DOWN]->zv=(400+ss)/sqrt(mass[2]); fret[2]=len/pow(pp,pluck[pc][1]);}
	 	if (pluck[pc][0]>-1){ n[DOWN]->av=(400+ss)/sqrt(mass[3]); fret[3]=len/pow(pp,pluck[pc][0]);}
		pc++;
		ss=0;
		if (pc>nc){go=0;}
	}
